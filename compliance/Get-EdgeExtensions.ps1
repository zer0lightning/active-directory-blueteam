...
$auditfilepath = "$($auditfolderpath)\MicrosoftEdge-$($env:USERNAME)-$($env:COMPUTERNAME).txt"
        ...
        if( $ExtensionId -and ($appid -eq $ExtensionId) ) {
            if( $Remove ) {
                ...
                if(!($WhatIf)) {
                    ...
                    if (Test-Path -Path "HKCU:\SOFTWARE\Microsoft\Edge\PreferenceMACs\Default\extensions.settings") {
                        if( Get-ItemProperty -Name "$appid" -Path "HKCU:\SOFTWARE\Microsoft\Edge\PreferenceMACs\Default\extensions.settings" ) {
                            Remove-ItemProperty -Name "$appid" -Path "HKCU:\SOFTWARE\Microsoft\Edge\PreferenceMACs\Default\extensions.settings"
                        }
                    }
                }
            } else {
